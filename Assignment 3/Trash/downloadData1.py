...
def changedatatype(df):
    ...
    df[['fico', 'cd_msa', 'mi_pct', 'cnt_borr', 'cnt_units', 'cltv', 'dti', 'orig_upb', 'ltv', 'zipcode',
        'orig_loan_term']] = df[
        ['fico', 'cd_msa', 'mi_pct', 'cnt_borr', 'cnt_units', 'cltv', 'dti', 'orig_upb', 'ltv', 'zipcode',
         'orig_loan_term']].astype('int64')
...
def changeperformancedatatype(perf_df):
    perf_df[['curr_ln_delin_status', 'loan_age', 'remng_mon_to_leg_matur', 'zero_bal_cd', 'current_dupb',
             'actual_loss_calc']] = perf_df[
        ['curr_ln_delin_status', 'loan_age', 'remng_mon_to_leg_matur', 'zero_bal_cd', 'current_dupb',
         'actual_loss_calc']].astype('int64')

    perf_df[['mon_rpt_prd', 'zero_bal_eff_dt', 'lst_pd_inst_duedt']] = perf_df[
        ['mon_rpt_prd', 'zero_bal_eff_dt', 'lst_pd_inst_duedt']].astype('str')
    return perf_df
...
def minmax(perf_df):
    ...
    new3_df = perf_df.groupby(['id_loan'])['curr_ln_delin_status'].min().to_frame(
        name='min_curr_ln_delin_status').reset_index()
    new4_df = perf_df.groupby(['id_loan'])['curr_ln_delin_status'].max().to_frame(
    ...
    new11_df = perf_df.groupby(['id_loan'])['non_mi_recoveries'].min().to_frame(
        name='min_non_mi_recoveries').reset_index()
    new12_df = perf_df.groupby(['id_loan'])['non_mi_recoveries'].max().to_frame(
    ...
    new17_df = perf_df.groupby(['id_loan'])['maint_pres_costs'].min().to_frame(
        name='min_maint_pres_costs').reset_index()
    new18_df = perf_df.groupby(['id_loan'])['maint_pres_costs'].max().to_frame(
    ...
    new23_df = perf_df.groupby(['id_loan'])['actual_loss_calc'].min().to_frame(
        name='min_actual_loss_calc').reset_index()
    new24_df = perf_df.groupby(['id_loan'])['actual_loss_calc'].max().to_frame(
    ...
    final_df = new1_df.merge(new2_df, on='id_loan').merge(new3_df, on='id_loan').merge(new4_df, on='id_loan').merge(
        new5_df, on='id_loan').merge(new6_df, on='id_loan').merge(new7_df, on='id_loan').merge(new8_df,
                                                                                               on='id_loan').merge(
        new11_df, on='id_loan').merge(new12_df, on='id_loan').merge(new13_df, on='id_loan').merge(new14_df,
                                                                                                  on='id_loan').merge(
        new15_df, on='id_loan').merge(new16_df, on='id_loan').merge(new17_df, on='id_loan').merge(new18_df,
                                                                                                  on='id_loan').merge(
        new19_df, on='id_loan').merge(new20_df, on='id_loan').merge(new21_df, on='id_loan').merge(new22_df,
                                                                                                  on='id_loan').merge(
        new23_df, on='id_loan').merge(new24_df, on='id_loan').merge(new25_df, on='id_loan').merge(new26_df,
                                                                                                  on='id_loan')
...
def constructcsv():
    ...
    with open(filename, 'w', encoding='utf-8', newline="") as f:
        for subdir, dirs, files in os.walk(download_path):
            for file in files:
                if 'time' not in file:
                    sample_df = pd.read_csv(os.path.join(subdir, file), sep="|",
                                            names=['fico', 'dt_first_pi', 'flag_fthb', 'dt_matr', 'cd_msa', "mi_pct",
                                                   ...
                                                   'id_loan', 'loan_purpose', 'orig_loan_term', 'cnt_borr',
                                                   'seller_name', 'servicer_name', 'flag_sc'], skipinitialspace=True)
        ...
        for subdir, dirs, files in os.walk(download_path):
            for file in files:
                if 'time_' in file:
                    temp_list = []
                    chunksize = 10 ** 6
                    for chunk in pd.read_csv(os.path.join(subdir, file), sep="|", skipinitialspace=True,
                                             chunksize=chunksize, low_memory=False, header=None):
                    ...
                    sample_df.columns = ['id_loan', 'mon_rpt_prd', 'current_aupb', 'curr_ln_delin_status', 'loan_age',
                                         'remng_mon_to_leg_matur', 'repurch_flag', 'mod_flag', 'zero_bal_cd',
                                         'zero_bal_eff_dt', 'current_int_rte', 'current_dupb', 'lst_pd_inst_duedt',
                                         'mi_recoveries', 'net_sale_proceeds', 'non_mi_recoveries', 'expenses',
                                         'legal_costs', 'maint_pres_costs', 'taxes_and_insur', 'misc_expenses',
                                         'actual_loss_calc', 'mod_cost']
                    ...
                    filtered_df['Year'] = ['19' + x if x == '99' else '20' + x for x in
                                           (filtered_df['id_loan'].apply(lambda x: x[2:4]))]
...
def main():
    ...
    trainQuarter=default['trainQtr']
    testQuarter=default['testQtr']
